...
public class CheckInPanel extends JPanel {
	...
	public JComboBox<String> dataServiceRequiredBox;
		...
		checkInButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (command.getSelectedRoom() == null) {
					// JOptionPane.showMessageDialog(container, "No room is
					// selected",
				...
				if (!ID.matches("[a-zA-Z][0-9]{8}")) {
					// JOptionPane.showMessageDialog(container, "The format of
					// the inputted ID is invalid. It should be an English
					// letter followed by exactly 8 digits",
					// "Input Error", JOptionPane.INFORMATION_MESSAGE);
					System.err.print(
							"Input Error: The format of the inputted ID is invalid. It should be an English letter followed by exactly 8 digits");
					return;
				}
				command.setID(ID);
				...
				try {
					...
					// JOptionPane.showMessageDialog(container, "The format of
					// the inputted check-in date is invalid",
					// "Input Error", JOptionPane.INFORMATION_MESSAGE);
				...
				if (text.matches("([0-9a-f]{2}:){5}[0-9a-f]{2}")) {
					...
					// JOptionPane.showMessageDialog(container, "The format of
					// the inputted ethernet address is invalid",
					// "Input Error", JOptionPane.INFORMATION_MESSAGE);
	...
	public void getCommandValues() {
		ArrayList<Room> list = hotelManager.listAllAvailableRooms();
		...
		availableRoomTable.setModel(new AbstractTableModel() {
			...
			public Object getValueAt(int r, int c) {
				...
				switch (c) {
				case 0:
					return room.getFloorNo() + "-" + room.getRoomNo();
				case 1:
					return room.getTypeString();
				case 2:
					return new Integer(room.getCapacity());
				case 3:
	...
	public JPanel getOccupantInfoHelper() {
		...
		occupantInfo.add(new JLabel("Occupant Details:"));
		...
		typeField = new JComboBox<String>(new String[] { "Standard", "Business" });
		...
		dataServiceRequiredBox = new JComboBox<String>(new String[] { "Yes", "No" });
