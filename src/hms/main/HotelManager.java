...
public class HotelManager {
	...
	public HotelManager() {
		...
		for (int i = 1; i <= noOfFloors; i++) {
			...
			for (int j = 1; j <= noOfRoomsInFloor; j++) {
				Room room = new Room(i, j, config.getRoomCapacity(i, j), config.getRoomType(i, j),
						config.getRoomRate(i, j));
				rooms[i - 1][j - 1] = room;
			}
		}
	 ...
	 * @param checkInDate
	 *            the check in date
	 * @param dataServiceRequired
	 *            the boolean indicate whether data service is required
	 ...
	 */
	public String checkIn(String ID, String name, String type, String company, Date checkInDate,
			boolean dataServiceRequired, String ethernetAddress, Room room) {
		if (ID == null || name == null || type == null || company == null || checkInDate == null
				|| ethernetAddress == null || room == null) {
	 ...
	 * @param value
	 *            the search target value
	 * @return the array list of search results
	 */
	public ArrayList<Room> findOccupant(String type, String value) {
		ArrayList<Room> result = new ArrayList<Room>();
